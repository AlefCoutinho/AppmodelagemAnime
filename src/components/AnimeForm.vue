                ...
                <v-form v-model="valid">
                    <v-row>
                        <v-col cols="12">
                            ...
                            <v-text-field label="Diretor" v-model="anime.diretor" ></v-text-field>
                            <v-text-field label="Estudio" v-model="anime.estudio" ></v-text-field>
                            <v-select v-model="anime.qtdtemporadas" :items="itemsQtdTemp" label="Quantidade de temporadas"> </v-select>
                            <v-textarea
                            ...
                            <v-select v-model="addCategoria" :items="categorias" label="Categorias"> </v-select>
                            <v-btn text @click="setCategoria">
                                <v-icon>mdi-plus</v-icon>
                                <span class="mr-2">Categoria</span>
                            </v-btn>
                            <ul> 
                                <li v-for="(cat, indice) in anime.categorias" :key="indice"> {{cat}}
                                    <v-btn class="red lighten-1 white--text" @click="removeCategoria(indice) "> <v-icon dark left>mdi-cancel  </v-icon></v-btn>
                                </li>
                                
                            </ul>

                            <v-select v-model="addPlataforma" :items="plataformas" label="Plataformas"> </v-select>
                            <v-btn text @click="setPlataforma">
                              ...
                              <ul> 
                                <li v-for="(plat, indice) in anime.plataformas" :key="indice">  {{plat}} 
                                    <v-btn class="red lighten-1 white--text" @click="removePlataforma(indice)"> <v-icon dark left>mdi-cancel  </v-icon></v-btn>
                                </li>
                                
                            </ul>

                            
                            <v-text-field type="date" label="Data de lançamento" v-model="anime.dtlancamento"  ></v-text-field>
                            <v-text-field type="number" label="Quantidade de episódios" v-model="anime.qtdEp"  ></v-text-field>
                            <v-select v-model="anime.statusDoAnime" :items="statusDoAnime" label="Status Do Anime"> </v-select>
                            <v-text-field type="number" label="Nota" v-model="anime.nota" ></v-text-field> 
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="12" class="text-right">
                            <v-btn class="primary" @click="salvarAnime">Salvar</v-btn>
                        </v-col>
                    </v-row>
                </v-form>
            </v-col>
                ...
                <v-simple-table>
                    ...
                    <tbody>
                        <tr v-for="(anime, indice) in listaAnime" :key="indice">
                            ...
                            <td>
                                <v-btn class="orange  lighten-1 white--text" @click="alterarAnime(anime)">
                                    <v-icon left>mdi-wrench </v-icon>
                                    Alterar
                                </v-btn>
                                <v-btn class="red lighten-1 white--text" @click="removerAnime(anime.id)">
...
export default {
    ...
    methods: {
        ...
        setPlataforma() {
            if (!this.anime.plataformas.includes(this.addPlataforma) && this.addPlataforma != "") {
